...
| **2b.5** | **Damage-engine state transitions § 2.7–2.9** — Winded threshold transitions, Dying state (death saves), Dead state, KO/unconscious. Prerequisite for 2b.6 and lifts the "hero death stops Malice generation" + "becoming winded triggers Fury and Troubadour gains" hooks left permissive in 2b.0 | `Participant.stamina` state machine; new intents (e.g. `SaveAgainstDeath`); event hooks for becoming-winded / dying / dead | 🟡 partially shipped via [Pass 3 Slice 1](superpowers/specs/2026-05-15-pass-3-slice-1-damage-state-machine-design.md) (state machine + per-trait overrides, `StaminaTransitioned` event hooks consumed by slice-2a class-δ subscribers). Death-save flow + KO/unconscious surface still open — needs an audit slice |
...
| **2b.9** | **Q10 cross-side ordering of simultaneous triggered actions** — when both a hero and a monster have a triggered action that fires on the same event, what order do they resolve in? Currently undefined in the engine | action-economy resolution rules | 🟡 partially shipped via [Pass 3 Slice 1](superpowers/specs/2026-05-15-pass-3-slice-1-damage-state-machine-design.md) (`ResolveTriggerOrder` intent + UI prompt for player picks). Audit needed to confirm full §4.10 coverage |
