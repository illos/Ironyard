...
| **2b.1** | **Attachment schema-shape gaps (ancestry-trait subset)** — per-echelon `stat-mod` scaling at the canonical 4th/7th/10th-level thresholds (Dwarf *Spark Off Your Skin* = +6/+12/+18/+24); `level + offset` immunity (Polder *Corruption Immunity* = `level + 2`; canon hardcodes the offset — "level+N" was an over-general framing) | `AttachmentEffect` schema: `stat-mod-echelon` variant + `immunity.value` extended with `{ kind: 'level-plus', offset: number }`; Polder Corruption Immunity + Spark Off Your Skin overrides re-authored | ✅ shipped via [Phase 2b Group A+B](superpowers/specs/2026-05-16-phase-2b-group-a-plus-b-design.md) (slices 3 + 4) on 2026-05-16. Wyrmplate + Psychic Scar dropped from scope per PS#1 — canon audit found both are level-scaling damage immunities (already correctly modeled in `ancestries.ts`), NOT per-echelon stat-mods. Item-side scaling moved to [Phase 2e](#phase-2e--items-engine-completeness) on 2026-05-16 |
...
| **2b.3** | **Kit completeness (kit-side only)** — three explicit sub-slices: **(a)** ranged-damage-bonus RollPower read-site verification; **(b)** `weapon-distance-bonus` (melee + ranged flavors; AoE-excluded; signature-aware via `cost === 0` gate); **(c)** `disengage-bonus` (13 kits with +1) — data-only ship (no Disengage intent, no OA suppression; bonus surfaces as a number for player adjudication at the table) | new `weapon-distance-bonus` + `disengage-bonus` effect kinds + parser regex + collector emission + UI surfaces (TurnFlowTab Move subtitle + Sheet kit section + ability-distance helper) | ✅ shipped via [Phase 2b Group A+B](superpowers/specs/2026-05-16-phase-2b-group-a-plus-b-design.md) (slices 10 + 11) on 2026-05-16. 2b.3.a turned out to be a no-op — RollPower already symmetric on melee/ranged branches per canon. Kit-keyword leveled-treasure bonuses plumbing moved to [Phase 2e](#phase-2e--items-engine-completeness) on 2026-05-16. No Disengage intent / OA suppression per `project_no_movement_tracking` memory + spec Q4 |
| **2b.4** | **Conditional / triggered attachments (ancestry-trait subset)** — hybrid runtime-eval seam (no `AttachmentCondition` extension): new `packages/rules/src/ancestry-triggers/` mirrors `class-triggers/`; per-encounter participant fields (`movementMode`, `bloodfireActive`) as serializable source of truth; read-site helpers (`getEffectiveWeaknesses`, `getEffectiveSpeed`) in new `packages/rules/src/effective.ts`. Devil/Dragon Knight *Wings* uses `movementMode = 'flying'` with Might-score duration + fall-on-Prone trigger + echelon-1 fire-weakness-5 read site. Orc *Bloodfire Rush* uses `bloodfireActive` latch on first-damage-this-round + EndRound clear + +2 effective speed. Polder *Shadowmeld* reuses `movementMode = 'shadow'`. Revenant *Bloodless* reclassified to `condition-immunity` shape in slice 2b.8 | new `StartFlying` / `EndFlying` / `SetMovementMode` / `SetBloodfireActive` intents; `ancestry-triggers/{wings,bloodfire}.ts`; `effective.ts` helpers; new participant fields `movementMode` + `bloodfireActive` | ✅ shipped via [Phase 2b Group A+B](superpowers/specs/2026-05-16-phase-2b-group-a-plus-b-design.md) (slices 6 + 7 + 8) on 2026-05-16. Item-side conditional/triggered (Color Cloak, Encepter, Mortal Coil) moved to [Phase 2e](#phase-2e--items-engine-completeness) |
...
| **2b.8** | **Q17B ancestry signature-trait engine gaps** — `condition-immunity` effect kind covering 6 ancestry traits (Bloodless = bleeding, Great Fortitude = weakened, Polder Fearless = frightened, Orc/Memonek Nonstop = slowed, High Elf Unstoppable Mind = dazed) + the `isImmuneToCondition` read-site helper; Memonek Unphased shipped as a separate `surprised`-flag gate (canonically a participant flag, not a ConditionType); 3 sig-traits riding the new ancestry-triggers infrastructure (Orc Relentless raises `orc-relentless-free-strike` OpenAction on dying-via-damage; Memonek Fall Lightly appends log-entry on fall paths in `end-flying`; Memonek Lightweight via `getSizeForForcedMove` helper); Polder Shadowmeld activates `StartFlying { mode: 'shadow' }` via `UseAbility` | new effect kinds: `condition-immunity`; new OpenAction kind `orc-relentless-free-strike`; new helpers `isImmuneToCondition` / `getSizeForForcedMove`; ancestry-triggers modules for `relentless` + `bloodfire` + `wings` + `fall-lightly` | ✅ shipped via [Phase 2b Group A+B](superpowers/specs/2026-05-16-phase-2b-group-a-plus-b-design.md) (slices 2 + 7 + 9) on 2026-05-16. **Glamors deferred** (PS#2): Wode Elf Glamor is canonically Hide/Sneak edge + contextual bane-on-others (needs trigger-cascade substrate from 2b.9); High Elf Glamor is Presence + Flirt/Persuade conditional; both signature traits need new emission infrastructure through `ancestries.ts` not present in `ancestry-traits.ts`. Bundled into a future follow-up sub-epic covering all signature-trait-emission gaps. Permanent-defer: Human *Detect the Supernatural*, Dwarf *Runic Carving* |
...
| **A + B. Combined Group A + B — conditional/triggered attachments + schema completeness batch** | 2b.1 + 2b.3 + 2b.4 + 2b.8 | Combined shipping group merged on 2026-05-16 because A's runtime-eval seam (no `AttachmentCondition` extension — hybrid via `ancestry-triggers/` mirror of `class-triggers/` + per-encounter participant fields + `effective.ts` read-site helpers) is the substrate 3 of B's sig-traits ride on. Shipped: Wings (Devil/DK movement-mode + duration + fall-on-Prone + echelon-1 fire-5), Bloodfire Rush (first-damage latch + +2 speed), Shadowmeld (mode='shadow'); condition-immunity (6 traits + Memonek Unphased via separate flag gate); per-echelon stat-mod (Spark Off Your Skin); level-plus immunity (Polder Corruption); weapon-distance-bonus (10 kits across melee + ranged); disengage-bonus UI surface (13 kits, data-only — no Disengage intent per `project_no_movement_tracking`); Orc Relentless OpenAction raise; Memonek Fall Lightly log entry; Memonek Lightweight read-site helper. **Deferred:** Glamors (signature-trait emission gap), Wyrmplate/Psychic Scar dropped (canon mismatch — they're level-scaling immunities, already correctly modeled). 10 sub-slices shipped + 1 deferred to follow-up | ✅ shipped 2026-05-16 across 11 commits on `phase-2b-group-ab` branch ([spec](superpowers/specs/2026-05-16-phase-2b-group-a-plus-b-design.md), [plan](superpowers/plans/2026-05-16-phase-2b-group-a-plus-b-plan.md)) |
