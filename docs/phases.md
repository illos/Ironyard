...
| **2b.13** | **Shipped-slice cleanup epic — Level-scaling for class heroic resources (P1, 6 bugs across 4-5 classes)** — per [2026-05-16 audit](superpowers/notes/2026-05-16-phase-2b-shipped-code-audit.md) cluster 3: B9 Censor wrath per-turn L7/L10 (Focused Wrath, Wrath of the Gods); B10 Censor Wrath Beyond Wrath @L4 dealer-side trigger; B11 Elementalist per-turn L7 + spatial-OA @L4 (Surging Essence, Font of Essence); B12 Conduit piety per-turn L7 (Faithful's Reward — `d3-plus` variant exists, only wired for Talent/Psion today); B22 Talent clarity force-move L4/L10 (Mind Recovery, Clear Mind); bonus Tactician focus per-turn L7/L10 (Heightened Focus, True Focus). One chokepoint refactor in `getResourceConfigForParticipant` for per-turn; level-aware factor passed to per-trigger amount calls in per-class evaluators | `heroic-resources.ts:getResourceConfigForParticipant`; per-class trigger evaluators (censor/elementalist/talent/tactician) | 🔴 P1 |
| **2b.14** | **Shipped-slice cleanup epic — Targeting-relations end-clauses (P1, 4 bugs / 2 root causes)** — per [2026-05-16 audit](superpowers/notes/2026-05-16-phase-2b-shipped-code-audit.md) cluster 5: B16/B27 Null Field + Tactician Mark don't clear when source enters dying state (canon: "ends if you are dying" / "until you are dying"); B24/B28 cross-Censor / cross-Tactician sweep — `mode: 'replace'` only clears the acting hero's own list, not other heroes' arrays containing the new target. Two centralized helpers: (a) `StaminaTransitioned → dying` hook clears source's `targetingRelations`; (b) `use-ability.ts` cascade emitter sweep over other participants' same-kind arrays | `class-triggers/stamina-transition.ts`; `intents/use-ability.ts` cascade emitter | 🔴 P1 |
| **2b.15** | **Shipped-slice cleanup epic — Damage-engine cleanup folds in here** — same scope as Group C from prior audit (2b.5 row punch-list) **plus** B30 (`title-doomed-opt-in` claim is no-op), B31 (rubble + inert lack `canRegainStamina:false`; ApplyHeal restores freely; canon: "can't regain Stamina or have this effect undone in any way"), B33 (RollPower fires Bleeding-d6 on any characteristic; canon only Might/Agility), B34 (3 BleedingTrigger discriminants have zero call sites — under-firing for non-roll main actions, free triggered abilities, standalone Might/Agility tests). Plus bugs B1, B2, B3 already in 2b.5 cleanup scope | `stamina.ts`, `damage.ts`, `apply-damage.ts`, `claim-open-action.ts:87-91`, `stamina-override.ts`, `roll-power.ts:181`, `condition-hooks.ts:165-169` | 🔴 P1 — **merge with 2b.5** |
| **2b.16** | **Shipped-slice cleanup epic — Other class-trigger bugs (P1-P3, 13 mixed items)** — per [2026-05-16 audit](superpowers/notes/2026-05-16-phase-2b-shipped-code-audit.md) cluster 6: B13 Elementalist 5×Reason auto-drop never fires (new event-source hook); B14 Maintenance multi-target cardinality (schema + reducer); B15 spatial-OA over-fire (cross-cutting latch-at-claim-time pattern; check Tactician + Troubadour for same shape); B17 Null side-check via `kind === 'monster'` instead of side; B18 `main-action-used` event fires on any slot toggle; B19 Shadow Insight fires on tier-damage-computed not delivered; B20 dead-code `spatial-trigger-null-field` OA branch; B21 Troubadour state-transition triggers lack `bodyIntact`/eligibility filter (latent); B23 Troubadour winded `cause === 'damage'` filter stricter than canon (verify); B25 Tactician marked-damage fires on ANY dealer (canon: "you or any ally"); B26 Psion-only toggles accepted from any PC (trust violation); B29 WS-mirror missing optimistic SetTargetingRelation cascade reflect; B32 `bodyIntact` never flipped to false (deferred per slice 1 spec). May split if too heavy | per-bug; mostly small surface | 🔴 P1-P3 — biggest by item count; can split |
...
| **CLEANUP-3. Other class-trigger bugs** | 2b.16 | 13 mixed P1-P3 items. Mostly small per-bug fixes; B13 (Elementalist 5×Reason auto-drop) is the biggest at "new event-source hook" complexity. May split if too heavy. Ride alongside any open group | Medium — one spec; may split |
