...
13. **Fury Ferocity per-round trigger corrected from 1d3 to +1 flat**: rules-canon two-gate review on 2026-05-15 caught the engine was emitting `requireFerocityD3(ctx)` for the per-round took-damage trigger, but canon (SC `Classes/Fury.md:90`, Heroes PDF p. ~10169) says +1 flat. Per-encounter stamina-transition triggers (first winded / first dying) correctly remain 1d3. Fixed in commit `c4fcea2`. § 5.4.4 flipped from 🚧 back to ✅.
