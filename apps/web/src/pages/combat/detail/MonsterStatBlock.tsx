  ...
  return (
    <div className="border border-line bg-ink-1 p-3 space-y-2">
      <div className="grid grid-cols-5 gap-1">
        {CHAR_LABELS.map(({ key, label }) => (
          <div key={key} className="border border-line bg-ink-2 px-1.5 py-2 text-center">
            <div className="font-mono text-[9px] uppercase tracking-[0.06em] text-text-mute">
              {label}
            </div>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
        <span>
          <span className="font-mono text-[10px] uppercase tracking-[0.06em] text-text-mute">
            Size
          </span>{' '}
        ...
        <span>
          ...
          <span className="font-mono tabular-nums">{fmt(speed)}</span>
        </span>
        <span>
          <span className="font-mono text-[10px] uppercase tracking-[0.06em] text-text-mute">
            Stab
          </span>{' '}
          <span className="font-mono tabular-nums">{fmt(stability)}</span>
        </span>
        <span>
          <span className="font-mono text-[10px] uppercase tracking-[0.06em] text-text-mute">
            Free Strike
          </span>{' '}
          <span className="font-mono tabular-nums">{fmt(freeStrike)}</span>
        </span>
        <span>
          <span className="font-mono text-[10px] uppercase tracking-[0.06em] text-text-mute">
            EV
          </span>{' '}
          <span className="font-mono tabular-nums">{fmt(ev)}</span>
        </span>
      </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
          {safeImmunities.length > 0 && (
            <span>
              ...
              <span className="font-mono">
                {safeImmunities.map((i) => `${i.type} ${i.value}`).join(' · ')}
              </span>
            </span>
          )}
          {safeWeaknesses.length > 0 && (
            <span>
              ...
              <span className="font-mono">
                {safeWeaknesses.map((w) => `${w.type} ${w.value}`).join(' · ')}
              </span>
            </span>
          )}
        </div>
      ...
      {withCaptain != null && (
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.06em] text-text-mute">
            With Captain
          </div>
