...
describe('class-triggers/per-class/null.evaluate', () => {
  ...
  it('raises spatial-trigger-null-field OA on enemy main-action-used', () => {
    ...
    expect(oa.payload).toEqual({
      ...
      participantId: 'null-1',
      expiresAtRound: null,
      payload: { actorId: 'mon-1', actorName: goblin.name },
    });
  });

  it('does NOT raise OA when nullFieldEnemyMainTriggered latch is true', () => {
    const nullPc = makeHeroParticipant('null-1', {
      className: 'Null',
      heroicResources: [{ name: 'discipline', value: 0, floor: 0 }],
    });
  ...
  it('does NOT raise OA when the main-action actor is a PC (ally), not a monster', () => {
    const nullPc = makeHeroParticipant('null-1', {
      className: 'Null',
      heroicResources: [{ name: 'discipline', value: 0, floor: 0 }],
    });
    const fury = makeHeroParticipant('fury-1', { className: 'Fury' });
    const state = stateWith([nullPc, fury]);
    const event: ActionEvent = { kind: 'main-action-used', actorId: 'fury-1' };
    expect(evaluateNull(state, event, testCtx)).toEqual([]);
  });
