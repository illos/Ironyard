...
// subscribers (see `packages/rules/src/class-triggers/`). All reducers in this
// file are no-cascade, no-log-noise: they mutate one field on one participant.
...
export function applySetParticipantPerRoundFlag(
  ...
  const parsed = SetParticipantPerRoundFlagPayloadSchema.safeParse(intent.payload);
  if (!parsed.success) {
    return {
      state,
      derived: [],
      log: [
        {
          kind: 'error',
          text: `SetParticipantPerRoundFlag rejected: ${parsed.error.message}`,
          intentId: intent.id,
        },
      ],
      errors: [{ code: 'invalid_payload', message: parsed.error.message }],
  ...
  if (!target || target.kind !== 'pc') {
    return {
      ...
      log: [
        {
          kind: 'error',
          text: `SetParticipantPerRoundFlag: participant ${participantId} not a PC`,
          intentId: intent.id,
        },
      ],
      errors: [{ code: 'target_missing', message: `PC participant ${participantId} not found` }],
